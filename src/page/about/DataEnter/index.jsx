...
const renderOption = (list) => list.map((item, index) => <Option key={index}
...
function Sensor({
  ...
  return <div>
    <Form form={configForm} onFinish={handleConfig} layout="inline">
      <Form.Item label="终端编号" name="terminalId" rules={[{
        required: true,
        message: '请输入projectId'
      }]}>
    ...
    <Divider/>
    <Table rowKey="sensorId" size="small" pagination={false} columns={columns}
...
function MonitorPoint({
  ...
  useEffect(() => {
    ...
    if (projectId) {
      monitorItemPoint.getMonitorItems(projectId)
        .then(({ data }) => {
